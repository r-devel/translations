...
1. Read about recent changes at https://docs.weblate.org/en/latest/changes.html
2. Check for special instructions at https://docs.weblate.org/en/latest/admin/upgrade.html#generic-upgrade-instructions
3. Backup up the data and database:
    ...
    docker exec weblate-docker_database_1 pg_dump -U weblate -Fc weblate > postgres-backup-$(date '+%Y-%m-%d').dump
    ```

4. Update the target Weblate version in the `docker-compose-https.yml` file, e.g.:
...
5. Actual Docker compose upgrade as per https://docs.weblate.org/en/latest/admin/install/docker.html#upgrading-docker
...
1. Attach the Docker container (this image is offered by `weblate`):
    ...
    Note that in some cases, you might want `root` access to the container, e.g. to install packages. As `sudo` is not available in the container, you need to `docker exec` using the `-u root` flag to get `root` access. Then install e.g.

    ```sh
    apt update
    apt install -y mc nano less
    ```
...
Currently @daroczig, @MichaelChirico, @azeloc, @beatrizmilz, @Saranjeet, and @hturner have admin access to the Weblate UI. SSH access is granted to @daroczig, @MichaelChirico,  @azeloc, and @beatrizmilz.
...
1. Select the user
1. Go to "Edit"
1. Mark the "Superuser status" checkbox
